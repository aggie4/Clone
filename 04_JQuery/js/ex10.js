...
pager.on({
    click: function () {
        let j = $(this).index(); /* 클릭한 순서값 저장 / j 는 지역함수*/
...
function moveToLeft(i) {
    let currentPhoto = photo.eq(s); /* 사진의 순서값 */
...
timer();
function timer() {
    /* 변수 autoplay는 멈춤 버튼을 클릭할 때에 사용해야 하기 때문에 전역변수로 선언이 되어야 한다. */
